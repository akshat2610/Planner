...
public class JournalBarGraph extends JPanel {
  public JournalBarGraph(View parentView, String title) {
    ...
    this.labels = new String[NUM_DAYS_PER_WEEK];
    this.values = new double[NUM_DAYS_PER_WEEK];
  ...
  public void refresh(){
    ...
    */
    resetLabels();
    ArrayList<Task> unfinishedTasks = new ArrayList<Task>(this.parentView.getModel().getUnfinishedTasks());
    int taskNdx = 0;
    //for each working day
    for (int i = 0; taskNdx < unfinishedTasks.size() && i < numDayWorkWeek; i++){
      //for each task for that day
